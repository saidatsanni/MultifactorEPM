...
load_data <- function(file = "./Datasets/qrtly_data_2020.xlsx") {
  ...
  data <- subset(data, Date_lag >= "1947-03-01" & Date_lag <= "2020-03-01")
...
# Run all regressions
run_regressions <- function(data, y = "QERET4", nw_lags = 8) {
  models <- get_models()
  results <- list()
...
bootstrap_pvalues <- function(data, predictors, y = "QERET4", nboot = 1000, nw_lags = 2) {
