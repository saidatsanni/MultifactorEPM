...
data_qtr$Date <- as.Date(data_qtr$Date, format= "%Y-%m-%d")
...
X<- data.frame(full_q$LPE_LAG,full_q$INFL_LAG)
...
#A. Define the ITERATIVE ALGORITHM
log_lik_iter<- function(k,x,y,r,beta) {
  vtm <-midas(r,k[1],k[2])[[1]]
  mu <- beta[1] + beta[2]*vtm + beta[3]*x[,1]+beta[4]*x[,2]
...
for (l in 1:length(grid1)){
  for (m in 1:length(grid2)){
    ...
    while((delta>=epsilon)&(step<50)){
      ...
      vtm <-midas(r,K[1],K[2])[[1]]
      wts <- 1/vtm
      vt_wls <- lm(Y ~ vtm+ X[,1]+X[,2],weights=wts)
...
##MIDAS VARIANCE GENERATION AND PREDICTIVE REGRESSIONS
...
##2. Obtain the initial values of the parameters vis weighted least squares
k1 = k1_it
k2 = k2_it
midasvar <- midas(r, k1,k2)[[1]]
wts <- 1/midasvar
mod_wls <- lm(QERET ~ midasvar+LPE_LAG+INFL_LAG, data= full_q, weights=wts)
...
# wts <- 1/vtmidas_opt
# vt_wls <- lm(QERET ~ vtmidas_opt+LPE_LAG+INFL_LAG,weights=wts, data= full_q)
# summary(vt_wls)

##5. Compute the standard error of beta using the sandwich formula
library(numDeriv)
vtm <- vtmidas_opt
mu <- er$par[1] + er$par[2]*vtm + er$par[3]*X[,1] + er$par[4]*X[,2]
...
log_lik_vec <- function(vec,u,y,v) {
  ...
  k2 <- vec[6]
  vtm <-midas(v,k1,k2)[[1]]
  mu <- b0 + b1*vtm + b2*u[,1] + b3*u[,2]
  logl_vec <- -(1/2)*log(vtm) - (1/2)*((y - mu)^2/vtm)
  return(logl_vec)
}

p = 4 # number of regression parameters
